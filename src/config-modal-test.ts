		...
		ctx: {
			...
			ui: {
				...
				async custom<T>(): Promise<T> {
					throw new Error(
						"custom UI should not be invoked in config-modal tests",
					);
				},
			},
...
runTest(
	"command completions return top-level and filtered RTK subcommands",
	() => {
...
await runAsyncTest(
	"config modal command handlers route RTK subcommands to controller actions",
	async () => {
		...
		assert.ok(
			lastNotification(infoCtx.notifications).message.includes("Usage: /rtk"),
		);

		await commandDefinition.handler("show", infoCtx.ctx);
		assert.ok(
			lastNotification(infoCtx.notifications).message.includes("mode=rewrite"),
		);

		await commandDefinition.handler("path", infoCtx.ctx);
		assert.equal(
			lastNotification(infoCtx.notifications).message,
			"rtk config: C:/tmp/pi-context-optimizer/config.json",
		);
		...
		assert.equal(
			lastNotification(infoCtx.notifications).message,
			"RTK integration settings reset to defaults.",
		);
		...
		assert.equal(
			...
			"/rtk requires interactive TUI mode.",
		);
	},
);
