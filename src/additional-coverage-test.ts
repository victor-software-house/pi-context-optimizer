...
runTest(
	"config-store normalizes invalid values and clamps numeric ranges",
	() => {
...
runTest(
	"config-store can ensure, save, and reload isolated config files",
	() => {
...
runTest("output metrics summarize tracked savings and clear state", () => {
	...
	assert.equal(
		getOutputMetricsSummary(),
		"RTK output compaction metrics: no data yet.",
	);

	const first = trackOutputSavings("1234567890", "12345", "bash", [
		"ansi",
		"truncate",
	]);
...
runTest(
	"command detection ignores env prefixes, blank lines, and chained suffixes",
	() => {
		assert.equal(
			normalizeCommandForDetection(
				"NODE_ENV=test FOO=bar npm test && echo done",
			),
			"npm test",
		);
		assert.equal(
		...
		assert.equal(
			matchesCommandPatterns("CI=1 bun test | head -5", [/^bun test/]),
			true,
		);
...
runTest(
	"path compaction preserves the tail and handles Windows separators",
	() => {
		const unixPath =
			"/Users/example/projects/pi-rtk-optimizer/src/techniques/path-utils.ts";
		...
		const windowsPath =
			"C:\\Users\\Administrator\\Documents\\pi-rtk-optimizer\\src\\windows-command-helpers.ts";
...
runTest(
	"windows bash compatibility rewrites only when the runtime is Windows",
	() => {
		const command =
			"cd /d C:\\Users\\Administrator\\project && python script.py";
		...
		if (process.platform === "win32") {
			...
			const alreadyUtf8 = applyWindowsBashCompatibilityFixes(
...
runTest(
	"rewrite pipeline safety buffers rewritten Windows producer commands",
	() => {
		...
		assert.equal(
			applyRewrittenCommandShellSafetyFixups("git diff | grep TODO"),
			"git diff | grep TODO",
		);
	},
);
