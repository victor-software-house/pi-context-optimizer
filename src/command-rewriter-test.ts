...
function expectedProxyExecutable(base: string): string {
	...
	return process.platform === "win32" && windowsExecutables.has(base)
		? `${base}.cmd`
		: base;
}

runTest(
	"pnpm dlx rewrites through RTK proxy instead of generic pnpm wrapper",
	() => {
		const config = cloneDefaultConfig();
		const decision = computeRewriteDecision(
			"pnpm dlx create-vite@latest demo --template react-ts",
			config,
		);
...
runTest(
	"docker run with shell command bypasses rewrite when interactive flags are missing",
	() => {
		const config = cloneDefaultConfig();
...
runTest(
	"docker compose exec without -it bypasses interactive shell rewrite",
	() => {
		const config = cloneDefaultConfig();
		const decision = computeRewriteDecision(
...
runTest(
	"container rewrites stay enabled for scripted shells and non-shell commands",
	() => {
		const config = cloneDefaultConfig();

		const scriptedShell = computeRewriteDecision(
			'docker run ubuntu bash -lc "echo hi"',
			config,
		);
		assert.equal(scriptedShell.changed, true);
		assert.equal(scriptedShell.rule?.id, "docker");
		assert.equal(
			scriptedShell.rewrittenCommand,
			'rtk docker run ubuntu bash -lc "echo hi"',
		);

		const nonShell = computeRewriteDecision(
		...
		assert.equal(
			nonShell.rewrittenCommand,
			"rtk docker run ubuntu python app.py",
		);
	},
);

runTest(
	"kubectl exec requires interactive flags before rewriting shell sessions",
	() => {
		const config = cloneDefaultConfig();
		const missingFlagsDecision = computeRewriteDecision(
			"kubectl exec pod-123 -- bash",
			config,
		);
		assert.equal(missingFlagsDecision.changed, false);
		assert.equal(
			missingFlagsDecision.rewrittenCommand,
			"kubectl exec pod-123 -- bash",
		);

		const interactiveDecision = computeRewriteDecision(
			"kubectl exec -it pod-123 -- bash",
			config,
		);
		assert.equal(interactiveDecision.changed, true);
		assert.equal(interactiveDecision.rule?.id, "kubectl");
		assert.equal(
			interactiveDecision.rewrittenCommand,
			"rtk kubectl exec -it pod-123 -- bash",
		);
	},
);

runTest(
	"sed scripts keep internal separators intact while later pipe segments still rewrite",
	() => {
		const config = cloneDefaultConfig();
		const decision = computeRewriteDecision(
			"sed -e s/a/b/;d file.txt | git status",
			config,
		);
		...
		assert.equal(
			decision.rewrittenCommand,
			"sed -e s/a/b/;d file.txt | rtk git status",
		);
...
runTest(
	"background operators rewrite both command segments without misreading redirect ampersands",
	() => {
		const config = cloneDefaultConfig();
		const backgroundDecision = computeRewriteDecision(
			"git status & cargo test",
			config,
		);
		assert.equal(backgroundDecision.changed, true);
		assert.equal(
			backgroundDecision.rewrittenCommand,
			"rtk git status & rtk cargo test",
		);

		const redirectDecision = computeRewriteDecision(
			"cargo test 2>&1 | head -5",
			config,
		);
		assert.equal(redirectDecision.changed, true);
		assert.equal(
			redirectDecision.rewrittenCommand,
			"rtk cargo test 2>&1 | head -5",
		);
...
runTest(
	"compound find and search shell flows bypass rewrites when RTK parity is unsafe",
	() => {
		const config = cloneDefaultConfig();
...
runTest(
	"native bash shell proxy flows bypass rewrites when RTK parity is unsafe",
	() => {
		const config = cloneDefaultConfig();
...
runTest("python proxy rewrites preserve the original executable token", () => {
	...
	assert.equal(
		decision.rewrittenCommand,
		`rtk proxy ${expectedProxyExecutable("python3.11")} -c "print(1)"`,
	);
});
